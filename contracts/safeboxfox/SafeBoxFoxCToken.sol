...
contract SafeBoxFoxCToken is
    SafeBoxFoxCTokenImpl,
    ...
    ISafeBox
{
    ...
    mapping(address => mapping(address => mapping(uint256 => uint256)))
        public borrowIndex; // _account, _strategy, _pid,  mapping id of borrowinfo, base from 1
    ...
    event SetOptimalUtilizationRate(
        uint256 oldV1,
        uint256 oldV2,
        uint256 newV1,
        uint256 newV2
    );
    event SetStableRateSlope(
        uint256 oldV1,
        uint256 oldV2,
        uint256 newV1,
        uint256 newV2
    ...
    function __SafeBoxFoxCToken_init(
        address _bank,
        address _cToken,
        address _devAddr
    ) public initializer {
    ...
    function getCATUserAmount(
        uint256 _pid,
        address _account
    ) external view virtual override returns (uint256 acctPoints) {
        _pid;
    ...
    function setOptimalUtilizationRate(
        uint256 _optimalUtilizationRate1,
        uint256 _optimalUtilizationRate2
    ) external onlyOwner {
        require(
            _optimalUtilizationRate1 <= 1e9 &&
                _optimalUtilizationRate2 <= 1e9 &&
                _optimalUtilizationRate1 < _optimalUtilizationRate2,
            "rate set error"
        );
        emit SetOptimalUtilizationRate(
            optimalUtilizationRate1,
            optimalUtilizationRate2,
            _optimalUtilizationRate1,
            _optimalUtilizationRate2
        );
        optimalUtilizationRate1 = _optimalUtilizationRate1;
        optimalUtilizationRate2 = _optimalUtilizationRate2;
    }

    function setStableRateSlope(
        uint256 _stableRateSlope1,
        uint256 _stableRateSlope2
    ) external onlyOwner {
        require(
            _stableRateSlope1 <= 1e4 * 1e9 &&
                _stableRateSlope1 >= 1e9 &&
                _stableRateSlope2 <= 1e4 * 1e9 &&
                _stableRateSlope2 >= 1e9,
            "rate set error"
        );
        emit SetStableRateSlope(
            stableRateSlope1,
            stableRateSlope2,
            _stableRateSlope1,
            _stableRateSlope2
    ...
    function borrowRatePerBlock() external view override returns (uint256) {
        return
            ctokenBorrowRatePerBlock().mul(getBorrowFactorPrewiew()).div(1e9);
    }

    function borrowInfoLength() external view override returns (uint256) {
    ...
    function getBorrowFactorPrewiew() public view virtual returns (uint256) {
    ...
    function _getBorrowFactor(
        ...
        uint256 value1 = stableRateSlope1
            .sub(1e9)
            .mul(borrowRate.sub(optimalUtilizationRate1))
        ...
        uint256 value2 = stableRateSlope2
            .sub(1e9)
            .mul(borrowRate.sub(optimalUtilizationRate2))
            .div(uint256(1e9).sub(optimalUtilizationRate2))
            .add(uint256(1e9));
        value = value2 > value1 ? value2 : value1;
    }

    function getBorrowTotal() public view virtual override returns (uint256) {
        return borrowTotalAmountWithPlatform;
    }

    function getDepositTotal() public view virtual override returns (uint256) {
        return totalSupply().mul(getBaseTokenPerLPToken()).div(1e18);
    }

    function getBaseTokenPerLPToken()
        public
        view
        virtual
        override
    ...
    function pendingSupplyAmount(
        address _account
    ) external view virtual override returns (uint256 value) {
        value = call_balanceOf(address(this), _account)
            .mul(getBaseTokenPerLPToken())
            .div(1e18);
    }

    function pendingBorrowAmount(
        uint256 _bid
    ...
    function pendingBorrowRewards(
        uint256 _bid
    ) public view virtual override returns (uint256 value) {
        if (borrowTotalPoints <= 0) {
            return 0;
        }
        value = borrowInfo[_bid].bPoints.mul(borrowTotalAmountWithPlatform).div(
            borrowTotalPoints
        );
    ...
    function _withdraw(
        ...
        uint256 maxBorrowAmount = call_balanceOfCToken_this()
            ...
            .div(1e9);
        require(
            maxBorrowAmount >= borrowTotalAmountWithPlatform,
    ...
    function getBorrowId(
        ...
        uint256 _pid,
        address _account
    ) public view virtual override returns (uint256 borrowId) {
    ...
    function _borrow(uint256 _bid, uint256 _value, address _to) internal {
        ...
        uint256 maxBorrowAmount = call_balanceOfCToken_this()
            ...
            .div(1e9);
        require(
            maxBorrowAmount >= borrowTotalAmountWithPlatform.add(_value),
            "no money to borrow"
        );
        ...
        if (borrowTotalPoints > 0) {
            addPoint = _value.mul(borrowTotalPoints).div(
                borrowTotalAmountWithPlatform
            );
        }

        borrowCurrent.bPoints = borrowCurrent.bPoints.add(addPoint);
        borrowTotalPoints = borrowTotalPoints.add(addPoint);
        borrowTotalAmountWithPlatform = borrowTotalAmountWithPlatform.add(
            _value
        );
        ...
        uint256 accountBorrowPointsOld = accountBorrowPoints[
            borrowCurrent.owner
        ];
        accountBorrowPoints[borrowCurrent.owner] = accountBorrowPoints[
            borrowCurrent.owner
        ].add(addPoint);
        ...
        if (compActionPool != address(0) && addPoint > 0) {
            IActionPools(compActionPool).onAcionIn(
                ...
                accountBorrowPoints[borrowCurrent.owner]
            );
        }
        return;
    }
    ...
    function _repay(uint256 _bid, uint256 _value) internal {
        ...
        if (_value >= pendingBorrowRewards(_bid).add(borrowCurrent.amount)) {
            ...
            removedPoints = _value.mul(borrowTotalPoints).div(
                borrowTotalAmountWithPlatform
            );
        ...
        // booking
        uint256 userAmount = removedPoints.mul(borrowCurrent.amount).div(
            borrowCurrent.bPoints
        ); // to reduce amount for booking
        uint256 repayAmount = removedPoints.mul(borrowTotalAmount).div(
            borrowTotalPoints
        ); // to repay = amount + interest
        uint256 platformAmount = TenMath.safeSub(
            removedPoints.mul(borrowTotalAmountWithPlatform).div(
                borrowTotalPoints
            ),
            repayAmount
        ); // platform interest

        borrowCurrent.bPoints = TenMath.safeSub(
            borrowCurrent.bPoints,
            removedPoints
        );
        borrowTotalPoints = TenMath.safeSub(borrowTotalPoints, removedPoints);
        borrowTotalAmountWithPlatform = TenMath.safeSub(
            borrowTotalAmountWithPlatform,
            repayAmount.add(platformAmount)
        );
        lastBorrowCurrent = call_borrowBalanceCurrent_this();

        borrowCurrent.amount = TenMath.safeSub(
            borrowCurrent.amount,
            userAmount
        );
        ...
        accountBorrowPoints[borrowCurrent.owner] = TenMath.safeSub(
            accountBorrowPoints[borrowCurrent.owner],
            removedPoints
        );
    ...
    function emergencyRepay(
        ...
        borrowTotalPoints = TenMath.safeSub(
            borrowTotalPoints,
            borrowCurrent.bPoints
        );
        borrowTotalAmount = TenMath.safeSub(borrowTotalAmount, repayAmount);
    ...
    function _update() internal {
        ...
        if (lastBorrowCurrentNow != lastBorrowCurrent && borrowTotal > 0) {
            if (lastBorrowCurrentNow >= lastBorrowCurrent) {
                // booking
                uint256 newDebtAmount1 = lastBorrowCurrentNow.sub(
                    lastBorrowCurrent
                );
                uint256 newDebtAmount2 = newDebtAmount1
                    .mul(getBorrowFactor())
                    .div(1e9);
                borrowTotalAmount = borrowTotalAmount.add(newDebtAmount1);
                borrowTotalAmountWithPlatform = borrowTotalAmountWithPlatform
                    .add(newDebtAmount2);
            }
        ...
        if (totalSupply() > 0 && accDebtPerSupply > 0) {
            ...
            uCTokenTotalAmount = uCTokenTotalAmount.add(
                accDebtPerSupply.mul(totalSupply()).div(1e18)
            );
        }
        if (uCTokenTotalAmount < totalSupply()) {
            // totalSupply() != 0  new debt divided equally
            accDebtPerSupply = accDebtPerSupply.add(
                totalSupply().sub(uCTokenTotalAmount).mul(1e18).div(
                    totalSupply()
                )
            );
        } else if (uCTokenTotalAmount > totalSupply() && accDebtPerSupply > 0) {
            // reduce debt divided equally
            uint256 accDebtReduce = uCTokenTotalAmount
                .sub(totalSupply())
                .mul(1e18)
                .div(totalSupply());
    ...
    function tokenSafeTransferAmount(
        address _token,
        uint256 _want,
        address _to
    ) internal {
