    ...
    ReentrancyGuardUpgradeable
{
    ...
    function addStrategy(
        address _strategylink,
        uint256 _pid,
        bool _blisted
    ) external onlyOwner {
        require(
            IStrategyLink(_strategylink).poolLength() > _pid,
            "not strategy pid"
        );
        strategyInfo.push(
            StrategyInfo(_blisted, IStrategyLink(_strategylink), _pid)
        );
        strategyIndex[_strategylink][_pid] = strategyInfo.length.sub(1);
        emit AddStrategy(
            strategyIndex[_strategylink][_pid],
            _strategylink,
            _pid,
            _blisted
        );
        require(
            IStrategyLink(_strategylink).bank() == address(this),
            "bank not me?"
        );
    }
    ...
    function depositLPToken(
        ...
        address lpToken = strategyInfo[_sid].iLink.getPoollpToken(
            strategyInfo[_sid].pid
        );
        IERC20Upgradeable(lpToken).safeTransferFrom(
            msg.sender,
            address(strategyInfo[_sid].iLink),
            _amount
        );
        ...
        return
            strategyInfo[_sid].iLink.depositLPToken(
                strategyInfo[_sid].pid,
                msg.sender,
                boxitem,
                _bAmount,
    ...
    function deposit(
        ...
        address[] memory collateralToken = strategyInfo[_sid]
            .iLink
            .getPoolCollateralToken(strategyInfo[_sid].pid);
        require(
            collateralToken.length == _amount.length,
            "_amount length error"
        );

        for (uint256 u = 0; u < collateralToken.length; u++) {
            if (_amount[u] > 0) {
                IERC20Upgradeable(collateralToken[u]).safeTransferFrom(
                    msg.sender,
                    address(strategyInfo[_sid].iLink),
                    _amount[u]
                );
        ...
        return
            strategyInfo[_sid].iLink.deposit(
                strategyInfo[_sid].pid,
                msg.sender,
                boxitem,
                _bAmount,
    ...
    function withdrawLPToken(
        uint256 _sid,
        uint256 _rate,
        uint256 _minOutputToken0,
        uint256 _minOutput
    ) external nonReentrant {
        return
            strategyInfo[_sid].iLink.withdrawLPToken(
                strategyInfo[_sid].pid,
                msg.sender,
                _rate,
                _minOutputToken0,
    ...
    function withdraw(
        ...
        return
            strategyInfo[_sid].iLink.withdraw(
                strategyInfo[_sid].pid,
                msg.sender,
                _rate,
                _toToken,
    ...
    function withdrawLPTokenAndClaim(
        uint256 _sid,
        uint256 _rate,
        uint256 _minOutputToken0,
        uint256 _minOutput,
        uint256 _poolClaimId,
    ...
    function emergencyWithdraw(
        uint256 _sid,
        uint256 _desirePrice,
        uint256 _slippage
    ) external nonReentrant {
        require(emergencyEnabled[_sid], "emergency not enabled");
        return
            strategyInfo[_sid].iLink.emergencyWithdraw(
                strategyInfo[_sid].pid,
                msg.sender,
                _desirePrice,
                _slippage
    ...
    function liquidation(
        ...
        if (_maxDebt > 0) {
            (address borrowFrom, ) = IStrategyLink(strategyInfo[_sid].iLink)
                .getBorrowInfo(pid, _account);
            address borrowToken = ISafeBox(borrowFrom).token();
            IERC20Upgradeable(borrowToken).safeTransferFrom(
                msg.sender,
                address(strategyInfo[_sid].iLink),
                _maxDebt
            );
        }
        strategyInfo[_sid].iLink.liquidation(
            pid,
            _account,
            msg.sender,
            _maxDebt
    ...
    function getBorrowAmount(
        ...
        value = strategyInfo[_sid].iLink.getBorrowAmount(
            strategyInfo[_sid].pid,
            _account
        );
    }
    ...
    function getDepositAmount(
        ...
        value = strategyInfo[_sid].iLink.getDepositAmount(
            strategyInfo[_sid].pid,
            _account
        );
    }
    ...
    function makeBorrowFrom(
        ...
        require(
            address(strategyInfo[sid].iLink) == msg.sender,
            "only call from strategy"
        );
        bid = ISafeBox(_borrowFrom).getBorrowId(
            msg.sender,
            _pid,
            _account,
            true
