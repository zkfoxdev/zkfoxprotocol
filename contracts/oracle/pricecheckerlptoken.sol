...
contract PriceCheckerLPToken is OwnableUpgradeable, IPriceChecker {
    ...
    function getLPTokenPriceInMdex(
        address _lptoken,
        address _t0,
        address _t1
    ...
    function getLPTokenPriceInOracle(
        address _t0,
        address _t1
    ) public view returns (uint256) {
    ...
    function checkLPTokenPriceLimit(
        ...
        uint256 currentSlip = uint256(price0.sub(price1).mul(1e9).div(price1));
        if (currentSlip > slip) {
            return false;
        }
        return true;
