...
contract ActionCompPools is OwnableUpgradeable, IActionPools, IClaimFromBank {
    ...
    // mint from foxtoken, when reward token is FOXToken  Token, mint it
    ...
    function add(
        ...
        (address lpToken, , uint256 totalPoints, ) = ICompActionTrigger(
            _callFrom
        ).getCATPoolInfo(_callId);
    ...
    function setMintTokens(
        ...
        for (uint256 i = 0; i < _mintTokens.length; i++) {
            require(
                IERC20Upgradeable(_mintTokens[i]).totalSupply() >= 0,
                "_mintTokens"
            );
    ...
    function setBooDev(address _dev) external {
    ...
    function getBlocksReward(
        ...
        if (pool.lastRewardClosed.add(value) > pool.poolTotalRewards) {
            value = pool.lastRewardClosed < pool.poolTotalRewards
                ? pool.poolTotalRewards.sub(pool.lastRewardClosed)
                : 0;
        }
    }
    ...
    function pendingRewards(
        ...
        uint256 userPoints = ICompActionTrigger(pool.callFrom).getCATUserAmount(
            pool.callId,
            _account
        );
        (, , uint256 poolTotalPoints, ) = ICompActionTrigger(pool.callFrom)
            .getCATPoolInfo(pool.callId);
    ...
    function pendingRewards(
        ...
        UserInfo storage user = userInfo[_pid][_account];
        value = totalRewards(_pid, _points, _totalPoints).add(
            user.rewardRemain
        );
    ...
    function totalRewards(
        ...
        if (block.timestamp > pool.lastRewardBlock && _totalPoints != 0) {
            uint256 poolReward = getBlocksReward(
                _pid,
    ...
    function updatePool(uint256 _pid) public {
        ...
        (, , uint256 poolTotalAmount, ) = ICompActionTrigger(pool.callFrom)
            .getCATPoolInfo(pool.callId);
        if (pool.rewardMaxPerBlock <= 0 || poolTotalAmount <= 0) {
        ...
        if (poolReward > 0) {
            ...
            if (mintTokens[rewardToken] > 0) {
                ...
                if (mintTokens[rewardToken] > 1e18) {
                    uint256 mintFee = poolReward
                        .mul(mintTokens[rewardToken].sub(1e18))
                        .div(1e18);
                    if (mintFee > 0) {
                        // IRewardsToken(rewardToken).mint(dev, mintFee); // mint for dev
                        mintFee; // delete mint for dev
                    }
                }
                pool.poolTotalRewards = pool.poolTotalRewards.add(poolReward);
                tokenTotalRewards[rewardToken] = tokenTotalRewards[rewardToken]
                    .add(poolReward);
            }
    ...
    function onAcionEmergency(uint256 _callId, address _account) external pure {
        _callId;
        _account;
    }
    ...
    function deposit(
        ...
        user.rewardRemain = pendingRewards(
            _pid,
            _account,
            _fromPoints,
            poolTotalPointsOld
        ...
        if (poolTotalPointsOld > 0) {
            poolDebt = TenMath.safeSub(
                pool.lastRewardTotal.mul(poolTotalPoints).div(
                    poolTotalPointsOld
                ),
                pool.lastRewardTotal
            );
        }
        ...
        if (poolTotalPoints > 0) {
            user.rewardDebt = pool.lastRewardTotal.mul(_toPoints).div(
                poolTotalPoints
            );
        }
    ...
    function withdraw(
        ...
        uint256 poolDebt = TenMath.safeSub(
            pool.lastRewardTotal,
    ...
    function _claim(
        ...
        if (value > 0) {
            ...
            pool.lastRewardClosed = TenMath.safeSub(
                pool.lastRewardClosed,
                value
            );

            if (rewardRestricted[_account] > 0) {
                value = TenMath.safeSub(
                    value,
                    value.mul(rewardRestricted[_account]).div(1e9)
                );
            }
            pool.poolTotalRewards = TenMath.safeSub(
                pool.poolTotalRewards,
                value
            );
            address rewardToken = address(pool.rewardToken);
            tokenTotalRewards[rewardToken] = TenMath.safeSub(
                tokenTotalRewards[rewardToken],
                value
            );
    ...
    function emergencyWithdraw(uint256 _pid, address _account) internal pure {
        _pid;
        _account;
    }
