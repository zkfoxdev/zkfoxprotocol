...
contract ZKFOXPools is OwnableUpgradeable, IActionTrigger, IClaimFromBank {
    ...
    function initialize(
        ...
        __Ownable_init();
        bank = _bank;
        rewardToken = ZKFOXToken(_rewardToken);
        devaddr = _devaddr;
    ...
    function getATUserAmount(
        uint256 _pid,
        address _account
    ) external view override returns (uint256 acctAmount) {
    ...
    function add(uint256 _allocPoint, address _lpToken) public onlyOwner {
        massUpdatePools();
        require(
            IERC20Upgradeable(_lpToken).totalSupply() >= 0,
            "error lptoken address"
        );
    ...
    function setClaimBeginTime(uint256 _claimBeginTime) external onlyOwner {
    ...
    function setPoolStartBlock(
        uint256 _pid,
        uint256 _startBlock
    ) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        require(block.timestamp < pool.lastRewardBlock, "set too late");
        require(_startBlock > block.timestamp, "startBlock error");
    ...
    function setEmergencyWithdrawEnabled(
        uint256 _pid,
        bool _set
    ) external onlyOwner {
    ...
    function totalRewards(
        ...
        if (block.timestamp > pool.lastRewardBlock && pool.totalAmount != 0) {
            uint256 poolReward = getBlocksReward(
                pool.lastRewardBlock,
                block.timestamp
            ).mul(pool.allocPoint).div(totalAllocPoint);
            accRewardPerShare = accRewardPerShare.add(
                poolReward.mul(1e18).div(pool.totalAmount)
            );
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        if (extendPool != address(0)) {
            IActionPools(extendPool).onAcionIn(
                _pid,
                msg.sender,
                amountOld,
                user.amount
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        if (extendPool != address(0)) {
            IActionPools(extendPool).onAcionOut(
                _pid,
                msg.sender,
                amountOld,
                user.amount
    ...
    function _claim(
        ...
        require(claimBeginTime <= block.timestamp, "not start");
