...
interface IStrategyConfig {
    ...
    event SetBorrowFactor(
        address _strategy,
        uint256 _poolid,
        uint256 _borrowFactor
    );
    event SetLiquidationFactor(
        address _strategy,
        uint256 _poolid,
        uint256 _liquidationFactor
    );
    event SetFarmPoolFactor(
        address _strategy,
        uint256 _poolid,
        uint256 _farmPoolFactor
    );
    event SetDepositFee(
        address _strategy,
        uint256 _poolid,
    ...
    function getFarmPoolFactor(
        address _strategy,
        uint256 _poolid
    ) external view returns (uint256 value);

    function setFarmPoolFactor(
        address _strategy,
        uint256 _poolid,
        uint256 _farmPoolFactor
    ) external;
