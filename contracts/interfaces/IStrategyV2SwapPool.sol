...
interface IStrategyV2SwapPool {
    ...
    function getPair(
        address _t0,
        address _t1
    ) external view returns (address pairs);
    ...
    function getLPTokenAmountInBaseToken(
        address _lpToken,
    ...
    function optimalBorrowAmount(
        address _lpToken,
        uint256 _amount0,
        uint256 _amount1
    ) external view returns (uint256 borrow0, uint256 borrow1);

    function optimalDepositAmount(
        address lpToken,
        uint amtA,
        uint amtB
    ) external view returns (uint swapAmt, bool isReversed);
